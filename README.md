...
4. **Draft** — Violations are compiled into a formal dispute notice with breach details and
   estimated penalties. The letter is generated by the LLM and fits any contract type.
...
# GROQ_API_KEY=gsk_...
# Real LLM activates automatically when GROQ_API_KEY is set.
...
- Select any `.txt` or `.pdf` SLA — AWS, ACME, telecom, logistics, any type
- Click **Extract Rules** — the LLM reads the contract and extracts enforceable rules

**Step 2 — Upload Vendor Logs**
- Upload any file from `data/logs_*.json`
- OR use the **Log Generator** helper — enter the incident month and downtime hours
- Click **Run Audit**

**Step 3 — Review & Approve**
- Violations appear as cards with breach details and penalty labels
- The LLM drafts a dispute letter appropriate to the contract type
...
and extracts the correct rules without any contract-specific configuration.
